...
BASH was developed by Brian Fox in 1988, and first released in 1989. Like programming languages, it can be used to automate tasks. Since it was one of the first computer programs ported by Linus Torvalds to Linux, it has become the default shell on most Linux distributions.
BASH supports a wide range of functionalities, such as using variables and arrays as data structures as well as special characters in `regular expressions`. By default, BASH reads user code one line at a time, but it can also read scripts containing multiple lines of code in the form of loops. Simple BASH scripts are plain text files with a `.sh` extension, and  can be executed in the terminal in a non-interactive way and whithout user intervention.

::::::::::::::::::::::::::::::::::::: callout

## Switching between Shells

It's entirely possible to switch between different shells. For example, if you are using ZSH and want to switch to BASH on a macOS, you can simply type `bash` in the terminal. To return to ZSH, type `zsh`. There are other shells available, such as `fish`(https://fishshell.com) for example. Fish is not POSIX compliant, but it has a lot of user-friendly features and it's quite similar to BASH. 
